...
\begin{tikzpicture}[
  ...
  \vertVec{}{xi}{$\xI{i}$};
  \node[ae trapezoid, right of=xi, node distance=2.4cm, rotate=-90] (enc) {\rotatebox{90}{$g_{\text{enc}}(\cdot;\theta_{e})$}};
  ...
  \node[ae trapezoid, above of=decMid, node distance=1.5cm, rotate=90] (decP) {\rotatebox{-90}{$\decP(\cdot;\theta_{p})$}};
  \vertVec{right of=decP}{xP}{$\xP$};
  \draw[net line] (decP) -- (xP);

  \node[ae trapezoid, below of=decMid, node distance=1.5cm, rotate=90] (decN) {\rotatebox{-90}{$\decN(\cdot;\theta_{n})$}};
